...
import { Temporal, TemporalJSON } from "./temporal"
...
const linkTemporals = (temporalArray: Temporal[]) => {
...
export const melody = (...args: (string | NoteInterface)[]) =>
 new Voice(
  args.map(
   (arg: string | NoteInterface) => new Verticality(arg as NoteInterface)
  )
 )

interface VoiceJSON {
 type: "voice"
 temporals: TemporalJSON[]
 position: number
}
...
export interface VoiceInterface {
 ...
 positions: number[]
 length: number
 ABIntervalSets: string[]
 mergedWith: (other: VoiceInterface) => VoiceInterface
 duplicate: () => VoiceInterface
 stripForTests: () => VoiceInterface
 durationAccordingToTimeSignature: (sig: TimeSignatureInterface) => number
 withoutNthTemporal: (n: number) => VoiceInterface // functional
 withOctaveAdjustedBy: (n: number) => VoiceInterface // functional
 at: (n: number) => VoiceInterface // stateful
 withDurations: (durs: DurationInterface[]) => VoiceInterface
 append: (other: VoiceInterface) => VoiceInterface // functional
 add: (t: Temporal) => VoiceInterface
 addAfterIdx: (t: Temporal, idx: number) => VoiceInterface
 addBeforeIdx: (t: Temporal, idx: number) => VoiceInterface
 deleteAtIdx: (idx: number, addMakeCurrentFlag: boolean) => VoiceInterface
 indexOf: (t: Temporal) => number
 deleteNFromEnd: (n: number) => VoiceInterface
 repeated: (n: number) => VoiceInterface
 toString: () => string
 toJSON: () => VoiceJSON
}
...
class Voice implements VoiceInterface {
 ...
 constructor(t = [] as Temporal[]) {
  this.temporals = t as Temporal[]
  this.position = 0
 }
 ...
 mergedWith(other: VoiceInterface): VoiceInterface {
  ...
  for (let i = 0; i < Math.max(lenA, lenB); i++) {
   ...
   if (a && b && a instanceof Verticality && b instanceof Verticality) {
   ...
   } else if (a && b && a instanceof Rest && b instanceof Verticality)
    merged.push(b)
   else if (a && b && b instanceof Rest && a instanceof Verticality)
    merged.push(a)
 ...
 get beats() {
  return this.temporals
   .map(temp => temp.duration.valueOf())
   .reduce((a, b) => a + b)
 }

 get ABIntervalSets() {
  return this.temporals
   .filter(temp => temp instanceof Verticality)
   .map(temp => (temp as Verticality).ABIntervalSet)
 }
 ...
 withOctaveAdjustedBy(n: number) {
  return new Voice(
   this.temporals.map(temp =>
    temp instanceof Verticality ? temp.withOctaveAdjustedBy(n) : temp
   )
  )
 }
 ...
 withDurations(durs: DurationInterface[]) {
  ...
  return new Voice(
   this.temporals.map((temp, idx) => {
    let dur = durs[idx]
     ? new Duration(durs[idx].dur.numerator, durs[idx].dur.denominator)
     : new Duration(
      durs[durs.length - 1].dur.numerator,
      durs[durs.length - 1].dur.denominator
     )
    if (temp instanceof Verticality) return new Verticality(temp.notes, dur)
    else if (temp instanceof Rest) return new Rest(dur)
    else {
     throw new Error("inexhaustive withDurations in Voice implementation")
    }
   })
  )
 }

 append(other: VoiceInterface) {
 ...
 addAfterIdx(temporal: Temporal, idx: number) {
  if (!isTemporal(temporal))
   throw new TypeError("cannot add a non-temporal to a voice")
 ...
 addBeforeIdx(temporal: Temporal, idx: number) {
  if (!isTemporal(temporal))
   throw new TypeError("cannot add a non-temporal to a voice")
  if (idx < 0 || idx >= this.temporals.length)
   throw new RangeError("invalid index into voice")
 ...
 deleteAtIdx(idx: number, addMakeCurrentFlag = false) {
  if (idx < 0 || idx >= this.temporals.length)
   throw new RangeError("invalid index into voice")
  if (idx === 0) {
 ...
 deleteNFromEnd(n: number) {
  if (n <= 0 || n > this.temporals.length) return this
